...
fn self_intersecting_shadow_preserves_both_winding_signs() {
    let outline = polygon(&[(1.125, 1.25), (14.75, 13.625), (2.25, 14.75), (13.5, 0.25)]);
...
fn a_shadow_preserves_a_hole_in_its_caster() {
    let mut outline = polygon(&[(1.25, 1.25), (15.75, 1.25), (15.75, 15.75), (1.25, 15.75)]);
...
fn nested_partial_clips_intersect_before_shadow_convolution() {
    let outline = polygon(&[(0.125, 0.125), (6.75, 0.125), (6.75, 7.625), (0.125, 7.625)]);
    let clips = [
        polygon(&[(0.125, 0.25), (7.875, 1.625), (1.75, 7.875)]),
        polygon(&[(0.375, 7.75), (3.625, 0.375), (7.625, 7.25)]),
    ];
    assert_shadow(&outline, Affine::translate((0.375, -0.125)), &clips, (9, 9));
}
...
fn affine_gaussian_impulses_match_oracle_including_degenerate_covariances() {
    ...
    exact[8 * size + 8] = 1.0;
    let source = super::coverage::Coverage::from_rows(
        8,
    ...
    for (sigma, transform) in [
        ...
        (
            1.25,
            Affine::rotate(0.4) * Affine::scale_non_uniform(2.0, 0.5),
        ),
