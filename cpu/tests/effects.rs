...
use cherenkov::{BlendMode, BlendSpace, Draw, Glyph, GlyphRun, GlyphStyle, Group,
    MeshGradient, Shadow, WorkingColor};
...
fn engine() -> Engine<Raster> {
    Engine::new(RasterConfig { threads: Some(2), ..RasterConfig::default() }).expect("engine")
}

fn render(engine: &Engine<Raster>, draw: impl FnOnce(&mut cherenkov::Recorder)) -> Vec<[f32; 4]> {
    let surface = engine.surface(Offscreen::new((48, 48), OffscreenFormat::LinearF32)).expect("surface");
    surface.update(|tx| { tx[surface.root()].content(surface.record(draw)); }).expect("update");
...
fn polygon(points: &[(f64, f64)]) -> BezPath {
    ...
    for &point in &points[1..] { outline.line_to(point); }
    outline.close_path();
    outline
}
...
fn assert_pixels(actual: &[[f32; 4]], expected: &[[f64; 4]], tolerance: f64) {
    ...
    for (index, (actual, expected)) in actual.iter().zip(expected).enumerate() {
        for channel in 0..4 {
            assert!((f64::from(actual[channel]) - expected[channel]).abs() < tolerance,
...
fn colored(field: &[f64], color: WorkingColor) -> Vec<[f64; 4]> {
    ...
    field.iter().map(|&area| [red * alpha * area, green * alpha * area,
        blue * alpha * area, alpha * area]).collect()
}

#[test]
fn arbitrary_shadow_rotation_offset_clips_and_cache_match_oracle() {
    let engine = engine();
    let caster = polygon(&[(4.125, 5.25), (31.75, 29.875), (3.25, 30.125), (32.5, 3.125)]);
    let clips = [polygon(&[(0.25, 0.5), (44.75, 4.25), (8.75, 43.5)]),
                ...
                recorder.clip(clips[0].clone(), |recorder| {
                    recorder.clip(clips[1].clone(), |recorder| {
                        recorder.transform(transform, |recorder| {
                            recorder.shadow(caster.clone(), Shadow::new(sigma, COLOR).offset(offset));
                        });
                    });
                });
            });
...
fn mesh_sampling_transforms_hdr_alpha_and_outside_match_oracle() {
    let mesh = MeshGradient::new(2, 1,
        vec![(2.0, 2.0).into(), (18.0, 3.0).into(), (35.0, 1.0).into(),
            (4.0, 35.0).into(), (22.0, 30.0).into(), (33.0, 38.0).into()],
        vec![COLOR, WorkingColor::new([9.0, 0.0, 0.0, 0.0]), WorkingColor::WHITE,
            WorkingColor::new([-0.25, 2.0, 0.0, 0.75]), COLOR, WorkingColor::WHITE]);
    let transform = Affine::translate((1.25, 2.5)) * Affine::rotate(-0.125);
    let actual = render(&engine(), |recorder| recorder.transform(transform, |recorder| {
        recorder.fill(Rect::new(-100.0, -100.0, 100.0, 100.0), mesh.clone());
    }));
    let expected: Vec<_> = (0..48_u32).flat_map(|y| (0..48_u32).map(move |x| (x, y)))
        .map(|(x, y)| cherenkov_oracle::mesh::sample(&mesh,
            transform.inverse() * Point::new(f64::from(x) + 0.5, f64::from(y) + 0.5))).collect();
...
fn encoded_groups_apply_opacity_before_compositing() {
    ...
    for (mode, reference) in [(BlendMode::Normal, cherenkov_scene::BlendMode::Normal),
        ...
        let actual = render(&engine, |recorder| {
            ...
            recorder.group(Group::new().blend(mode).blend_space(BlendSpace::SrgbEncoded).opacity(0.625),
        ...
        let expected = cherenkov_oracle::blend::in_space(reference, BlendSpace::SrgbEncoded, destination, source);
...
fn encoded_layer_matches_an_encoded_group() {
    ...
    surface.update(|tx| {
        ...
        tx[&child].blend_space(BlendSpace::SrgbEncoded).opacity(0.5)
            .content(surface.record(|recorder| recorder.fill(Rect::new(0.0, 0.0, 48.0, 48.0), COLOR)));
    }).expect("update");
    engine.render(FrameTime::now()).expect("render");
    let expected = cherenkov_oracle::blend::in_space(cherenkov_scene::BlendMode::Normal,
        BlendSpace::SrgbEncoded, [1.0; 4], colored(&[0.5], COLOR)[0]);
    assert_pixels(&surface.readback().expect("readback").pixels, &vec![expected; SIZE * SIZE], 2e-5);
}
...
fn transformed_stroked_glyphs_and_cache_identity_match_oracle() {
    ...
    let mut run = GlyphRun { font: font.id(), size: 30.0, coords: Vec::new(),
        glyphs: vec![Glyph { id: 36, x: 17.125, y: 33.75,
    ...
    for style in [GlyphStyle::Fill,
        GlyphStyle::Stroke(Stroke::new(1.5).with_join(kurbo::Join::Miter)),
        GlyphStyle::Stroke(Stroke::new(2.25).with_join(kurbo::Join::Round)
            .with_caps(kurbo::Cap::Round).with_dashes(0.5, [2.0, 1.0])), GlyphStyle::Fill] {
        run.style = style;
        let outline = cherenkov_oracle::glyphs::styled_outline(data, 0, &run, &run.glyphs[0], Affine::IDENTITY)
            .expect("oracle outline");
        let edges = cherenkov_oracle::clip::intersect_edges(&path::edges(&path::flatten(&outline)),
            FillRule::NonZero, &clip_edges);
        let mut coverage = Coverage::new(SIZE, SIZE);
        for (x0, y0, x1, y1) in edges { coverage.add_line(x0, y0, x1, y1); }
        let expected = colored(&coverage.finish(FillRule::NonZero), COLOR);
        let actual = render(&engine, |recorder| recorder.clip(clip.clone(), |recorder| recorder.glyphs(&run, COLOR)));
...
fn a_transformed_colour_glyph_matches_the_oracle_paint_graph() {
    ...
    let font = engine.font(FontSource::bytes(data.to_vec())).expect("colour font");
    let local = Affine::rotate(0.125);
    let glyph = Glyph { id: 1, x: 13.25, y: 35.5, transform: Some(local) };
    let run = GlyphRun { font: font.id(), size: 30.0, coords: Vec::new(),
        glyphs: vec![glyph], style: GlyphStyle::Fill };
    let actual = render(&engine, |recorder| recorder.glyphs(&run, COLOR));
    let mut scene = cherenkov_scene::Scene::new(48, 48,
    ...
    scene.root.items.push(cherenkov_scene::Item::Draw(cherenkov_scene::Draw::Glyphs(
        cherenkov_scene::GlyphRun {
            font: cherenkov_scene::ResourceHash::of(data), font_index: 0, size: 30.0,
            normalized_coords: Vec::new(),
            glyphs: vec![cherenkov_scene::Glyph { id: 1, x: 0.0, y: 0.0 }],
            paint: cherenkov_scene::Color::new(cherenkov_scene::ColorSpace::LinearP3, COLOR.components).into(),
        })));
    let reference = cherenkov_oracle::render::Renderer::new(SIZE, SIZE)
        .render(&scene, std::path::Path::new("../scenes/corpus/text-colr")).expect("oracle colour glyph");
    let expected: Vec<_> = reference.pixels.iter().map(|pixel| pixel.map(f64::from)).collect();
...
fn root_layer_state_uses_the_same_compositor_as_child_layers() {
    ...
    let surface = engine.surface(Offscreen::new((48, 48), OffscreenFormat::LinearF32)).expect("surface");
    surface.update(|tx| {
        tx[surface.root()].blend_space(BlendSpace::SrgbEncoded).opacity(0.5)
            .transform(Affine::translate((2.0, 3.0)))
            .content(surface.record(|recorder| recorder.fill(Rect::new(0.0, 0.0, 1.0, 1.0), COLOR)));
    }).expect("update");
    engine.render(FrameTime::now()).expect("render");
