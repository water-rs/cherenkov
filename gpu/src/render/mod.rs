...
impl Renderer {
    ...
    fn render_frame(&mut self) -> Result<(Next, FrameStats), RenderError> {
        ...
        // Lower every dirty surface into the shared buffers first. Instances,
        // stops and globals are appended frame-wide at per-surface bases so a
        // later surface's upload can't clobber an earlier one before it is
        // encoded.
        ...
        tracing::debug!(
            surfaces = dirty.len(),
            lower_ms = lower_start.elapsed().as_secs_f64() * 1e3,
            ok = result.is_ok(),
            "frame lowered"
        );
        if result.is_ok() {
        ...
        if self.timestamps && self.frame_pass_count > 0 {
            let ticks = self.resolve_timestamps(2 * self.frame_pass_count)?;
            ...
            stats.gpu_seconds = delta(0, 2 * self.frame_pass_count as usize - 1);
            ...
            tracing::debug!(
                passes = self.frame_pass_count,
                gpu_ms = stats.gpu_seconds.map(|s| s * 1e3),
                "frame timed"
            );
        }
        self.wait("the frame's passes")?;
        result?;
        Ok((Next::Idle, stats))
    }
    ...
    /// Blocks until every submission so far has completed, for at most
    /// [`Renderer::wait_timeout`]; `what` names the work for the error.
    fn wait(&self, what: &'static str) -> Result<(), RenderError> {
    ...
    /// Maps `slice` for reading and blocks (bounded) until the map result
    /// arrives; `what` names the readback for tracing and errors.
    fn map_read(
        &self,
    ...
    fn resolve_timestamps(&self, count: u32) -> Result<Vec<u64>, RenderError> {
        ...
        self.map_read(slice, "the timestamp readback")?;
    ...
    fn readback(&self, surface: SurfaceId) -> Result<Readback, RenderError> {
        ...
        let slice = buf.slice(..);
        self.map_read(slice, "the pixel readback")?;
        let data = slice.get_mapped_range();
