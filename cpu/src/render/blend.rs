...
#[expect(
    ...
    reason = "avoid four software fmaf calls per composited pixel on baseline targets"
)]
        ...
        for mode in modes {
            for (cb, cs) in &cases {
                for space in [
                    ...
                    let tolerance = if space == cherenkov::BlendSpace::Linear {
                        1e-5
                    } else {
                        2e-5
                    };
                    for channel in 0..4 {
                        assert!(
                            (f64::from(got[channel]) - want[channel]).abs() < tolerance,
