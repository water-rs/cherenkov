...
impl Offscreen {
    ...
    /// The target's readback format.
    #[must_use]
    pub const fn format(self) -> OffscreenFormat {
        self.format
    }
}

/// The materialization format of an offscreen target's readback.
///
/// The framebuffer is always f32 premultiplied linear Display P3;
/// the format selects what a readback reports.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum OffscreenFormat {
    /// Premultiplied linear Display P3, 16-bit float per channel: each f32
    /// channel is rounded through `half::f16` and back — the canonical
    /// materialization of `docs/api.md`.
    #[default]
    LinearF16,
    /// Premultiplied linear Display P3, f32 per channel: the raw
    /// framebuffer with no f16 rounding.
    LinearF32,
}
...
pub struct FrameStats {
    /// GPU seconds the frame took. Always `None` for the CPU backend.
    pub gpu_seconds: Option<f64>,
    /// Render passes recorded (bands processed).
    pub passes: u32,
    /// Draw items rasterized.
    pub draws: u32,
    /// Edges rasterized.
    pub instances: u32,
    /// Glyphs rasterized into the mask cache this frame.
...
impl Surface {
    ...
    ///
    /// # Errors
    /// [`SurfaceError::TooLarge`] when a dimension exceeds the framebuffer
