    ...
    for row in (0..mesh.rows() as usize).rev() {
        for column in (0..mesh.columns() as usize).rev() {
            ...
            for (index, weight) in indices.into_iter().zip(weights) {
                ...
                for channel in 0..3 {
                    color[channel] = (channels[channel] * channels[3]).mul_add(weight, color[channel]);
                }
...
mod tests {
    ...
    fn bilinear_premultiplication_and_outside_domain() {
        let mesh = MeshGradient::new(1, 1,
            vec![(0.0, 0.0).into(), (2.0, 0.0).into(), (0.0, 2.0).into(), (4.0, 2.0).into()],
            vec![WorkingColor::new([8.0, 0.0, 0.0, 0.0]),
        ...
        assert!(sample(&mesh, Point::new(-1.0, 1.0)).iter().all(|v| v.abs() < 1e-12));
    }

    #[test]
    fn inverse_handles_reflection_collapse_and_two_roots() {
        let corners = [Point::ORIGIN, Point::new(1.0, 0.0), Point::new(0.0, 1.0), Point::new(-1.0, -1.0)];
