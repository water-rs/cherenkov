...
pub fn spread_coverage(
    ...
    let path = if let Some(rounded) = rounded {
        ...
        if rect.width() <= 0.0 || rect.height() <= 0.0 { return vec![0.0; size.0 * size.1]; }
        let radii = rounded.radii();
        let radius = |r: f64| if r > 0.0 { (r + spread).max(0.0) } else { 0.0 };
        kurbo::RoundedRect::from_rect(rect, kurbo::RoundedRectRadii::new(
    ...
    if spread == 0.0 || rounded.is_some() { return original; }
...
impl Gaussian {
    #[expect(clippy::suboptimal_flops, reason = "separate products preserve exact zero covariance and determinant for orthogonal or singular matrices")]
    fn new(sigma: f64, transform: kurbo::Affine) -> Self {
        ...
        } else { sigma * b.hypot(d) };
        Self { sx: sigma * horizontal, sy: sigma * b.hypot(d), slope, residual }
    }
    ...
    fn tap(&self, x: f64, y: f64) -> f64 {
        ...
        cuts.windows(2).map(|interval| {
            integrate(&density, interval[0], interval[1], 2e-15, 24)
        }).sum()
    }
}

fn axis_tap(distance: f64, sigma: f64) -> f64 {
    if sigma == 0.0 { return if distance == 0.0 { 1.0 } else { 0.0 }; }
...
pub fn gaussian_blur(
    src: &[f64], width: usize, height: usize, sigma: f64, transform: kurbo::Affine,
) -> Vec<f64> {
    ...
    if gaussian.slope == 0.0 {
        ...
        for (radius, sigma, stride) in [(rx, gaussian.sx, (1, 0)), (ry, gaussian.sy, (0, 1))] {
            let weights: Vec<_> = (-radius..=radius).map(|i| axis_tap(i as f64, sigma)).collect();
            ...
            for y in 0..height {
                for x in 0..width {
                    ...
                    for (i, &weight) in weights.iter().enumerate() {
                        ...
                        let column = (x as i64 + distance * stride.0).clamp(0, width as i64 - 1) as usize;
                        let row = (y as i64 + distance * stride.1).clamp(0, height as i64 - 1) as usize;
    ...
    for y in -ry..=ry {
        for x in -rx..=rx {
            ...
            if weight > 0.0 { taps.push((x, y, weight)); }
        }
    }
...
mod tests {
    ...
    fn signed_miter_spread_has_the_expected_rect_area() {
        ...
        for shape in [Shape::Rect(rect), Shape::Path { path: rect.to_path(1e-4) }] {
            for (spread, area) in [(-5.0, 0.0), (-1.5, 49.0), (0.0, 100.0), (1.5, 169.0)] {
                let field = spread_coverage(&shape, FillRule::NonZero,
                    Affine::IDENTITY, spread, &[], (24, 24));
    ...
    fn rounded_spread_keeps_sharp_corners_and_clamps_shrinking_radii() {
        let shape = Shape::RoundedRect(RoundedRect::from_rect(Rect::new(6.0, 6.0, 18.0, 18.0),
        ...
        for (spread, side, squared_radii) in [(-2.0, 8.0_f64, 5.0), (2.0, 16.0, 70.0)] {
            ...
            if spread > 0.0 { assert!((field[4 * 24 + 4] - 1.0).abs() < 1e-12); }
        }
    }
    ...
    fn beyond_limit_miters_are_beveled() {
        ...
        path.close_path();
        let field = spread_coverage(&Shape::Path { path }, FillRule::NonZero,
            Affine::IDENTITY, 2.0, &[], (24, 24));
    ...
    fn integrated_taps_follow_scale_rotation_reflection_and_translation() {
        ...
        for transform in [Affine::scale(3.0), Affine::rotate(0.7) * Affine::scale(3.0),
            Affine::new([-3.0, 0.0, 0.0, 3.0, 123.0, -50.0])] {
            let actual = gaussian_blur(&source, 5, 1, 0.5, transform);
            for (&a, &b) in actual.iter().zip(&expected) { assert!((a - b).abs() < 1e-13); }
        }
    ...
    fn correlated_integrals_match_the_analytic_quadrant_probability() {
        for correlation in [-0.999_999_999_999_f64, -0.6, 0.6, 0.999_999_999_999] {
            ...
            let gaussian = Gaussian::new(0.025, Affine::new([1.0, correlation, 0.0, residual, 0.0, 0.0]));
    ...
    fn tiny_local_sigma_is_not_discarded_before_magnification() {
        ...
        for (&a, &b) in actual.iter().zip(&expected) { assert!((a - b).abs() < 1e-14); }
        assert!(actual[0] > 0.2);
    }
    ...
    fn sheared_impulse_has_the_transformed_covariance() {
        ...
        source[16 * 33 + 16] = 1.0;
        let field = gaussian_blur(&source, 33, 33, 2.0, Affine::new([1.0, 0.6, 0.0, 0.8, 0.0, 0.0]));
