...
fn run() -> GlyphRun {
    GlyphRun {
        font: FontId::new(1), size: 40.0, coords: Vec::new(),
        glyphs: vec![Glyph { id: 36, x: 13.25, y: 48.5, transform: None }],
...
fn glyph_transform_is_about_the_origin_in_run_units() {
    ...
    let mut run = run();
    let initial = styled_outline(bytes, 0, &run, &run.glyphs[0], Affine::IDENTITY).expect("outline");
    let local = Affine::new([0.0, 1.5, -0.75, 0.0, 2.25, -1.125]);
    run.glyphs[0].transform = Some(local);
    let actual = styled_outline(bytes, 0, &run, &run.glyphs[0], Affine::IDENTITY).expect("transformed outline");
    ...
    for (value, expected) in [actual_bounds.x0, actual_bounds.y0, actual_bounds.x1, actual_bounds.y1]
        .into_iter().zip([expected_bounds.x0, expected_bounds.y0, expected_bounds.x1, expected_bounds.y1]) {
