...
impl Gaussian {
    ...
    fn tap(&self, x: f64, y: f64) -> f64 {
        ...
        cuts.windows(2).map(|interval| {
            integrate(&density, interval[0], interval[1], 2e-14, 20)
        }).sum()
    }
}
...
fn panel(f: &impl Fn(f64) -> f64, low: f64, high: f64) -> f64 {
    ...
    for (node, weight) in [
        ...
        value = weight.mul_add(f(node.mul_add(half, mid)) + f((-node).mul_add(half, mid)), value);
    }
    value * half
}
...
fn integrate(f: &impl Fn(f64) -> f64, low: f64, high: f64, tolerance: f64, depth: u32) -> f64 {
    ...
    if (halves - whole).abs() <= tolerance { return halves; }
    ...
    if gaussian.slope == 0.0 {
        let axis = |radius: usize, sigma| {
            let mut weights: Vec<_> = (0..=2 * radius)
                .map(|i| axis_tap(i as f64 - radius as f64, sigma)).collect();
            let mass: f64 = weights.iter().sum();
            for weight in &mut weights { *weight /= mass; }
            weights
        };
        return Kernel::Separable {
            horizontal: axis(radius_x, gaussian.sx), vertical: axis(radius_y, gaussian.sy),
        };
    }
    let mut rows: Vec<Vec<_>> = (0..=2 * radius_y).map(|y| {
        (0..=2 * radius_x).map(|x| gaussian.tap(
            x as f64 - radius_x as f64, y as f64 - radius_y as f64)).collect()
    }).collect();
    let mass: f64 = rows.iter().flatten().sum();
    for weight in rows.iter_mut().flatten() { *weight /= mass; }
