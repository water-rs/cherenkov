...
pub fn box_shape(shape: &ShapeData) -> Result<Option<Boxed>, Encode> {
...
impl Lowered {
    pub fn full(
        ...
        let mut ops = Vec::new();
        let mut spans = vec![0..0; list.len()];
        lower_commands(list, 0, list.len(), Affine::IDENTITY, &mut ops, &mut spans, fonts, images)?;
        Ok(Self { ops, spans })
    }

    /// Patches stable command spans. A changed op count or scope kind is
    /// a structural edit: rebuild this layer, never another layer.
    pub fn patch(
        &mut self,
        list: &DisplayList,
        dirty: &Dirty,
        fonts: &HashMap<u64, FontData>,
        images: &HashMap<u64, GpuImage>,
    ) -> Result<Option<Vec<Range<usize>>>, Encode> {
        let mut patches = Vec::new();
        for range in dirty.ranges() {
            let (a, b) = (range.start as usize, range.end as usize);
            let mut ops = Vec::new();
            let mut spans = vec![0..0; list.len()];
            lower_commands(list, a, b, ambient_transform(list, a), &mut ops, &mut spans, fonts, images)?;
            let lo = self.spans[a].start as usize;
            let hi = self.spans[b - 1].end as usize;
            if hi - lo != ops.len() || self.ops[lo..hi].iter().zip(&ops).any(|(old, new)| {
                std::mem::discriminant(old) != std::mem::discriminant(new)
                    || matches!((old, new), (Op::Glyphs { run: a, .. }, Op::Glyphs { run: b, .. }) if a.glyphs.len() != b.glyphs.len())
            }) {
                *self = Self::full(list, fonts, images)?;
                return Ok(None);
            }
            ...
            for (dst, src) in self.spans[a..b].iter_mut().zip(&spans[a..b]) {
                *dst = src.start + offset..src.end + offset;
            }
            patches.push(lo..hi);
        }
        Ok(Some(patches))
...
fn lower_list(
    ...
    let mut trash = vec![0..0; list.len()];
    ...
    fn commands(
        ...
        while i < end {
            ...
            match &commands[i] {
                ...
                Command::Stroke { shape, stroke, paint } => self.stroke(ambient, shape, stroke, paint, ops)?,
                ...
                Command::Image { image, dst, sampling } => self.image_draw(ambient, *image, dst, *sampling, ops)?,
                Command::Picture { picture, transform } => lower_list(self, picture.display_list(), ambient * *transform, ops)?,
                Command::BeginClip { shape, end } => {
                    ops.push(Op::BeginClip { local: ambient, shape: clip_shape(shape)?, end: 0 });
                    spans[i] = start..start + 1;
                    self.commands(list, i + 1, *end as usize, ambient, ops, spans)?;
                    let close = u32::try_from(ops.len()).expect("op index fits u32");
                    if let Op::BeginClip { end, .. } = &mut ops[start as usize] { *end = close; }
                    ops.push(Op::End);
                    spans[*end as usize] = close..close + 1;
                    i = *end as usize + 1;
                    continue;
                }
                Command::BeginTransform { transform, end } => {
                    spans[i] = start..start;
                    self.commands(list, i + 1, *end as usize, ambient * *transform, ops, spans)?;
                    let close = u32::try_from(ops.len()).expect("op index fits u32");
                    spans[*end as usize] = close..close;
                    i = *end as usize + 1;
                    continue;
                }
                Command::BeginGroup { group, end } => {
                    if group.filter.is_some() { return Err(RenderError::Unsupported(names::FILTER).into()); }
                    if group.blend_space != BlendSpace::Linear { return Err(RenderError::Unsupported(names::BLEND_SPACE).into()); }
                    let isolate = group.opacity < 1.0 || group.blend != BlendMode::Normal;
                    if isolate { ops.push(Op::BeginIsolate { opacity: group.opacity, blend: group.blend, end: 0 }); }
                    spans[i] = start..u32::try_from(ops.len()).expect("op index fits u32");
                    self.commands(list, i + 1, *end as usize, ambient, ops, spans)?;
                    let close = u32::try_from(ops.len()).expect("op index fits u32");
                    if isolate {
                        if let Op::BeginIsolate { end, .. } = &mut ops[start as usize] { *end = close; }
                        ops.push(Op::End);
                    }
                    spans[*end as usize] = close..u32::try_from(ops.len()).expect("op index fits u32");
                    i = *end as usize + 1;
                    continue;
                }
                Command::End => unreachable!("validated scope ends are consumed by their opener"),
            }
            spans[command_index] = start..u32::try_from(ops.len()).expect("op index fits u32");
            i += 1;
        }
        Ok(())
    }
